...
- Profiles can pick their own icon — any lucide glyph from a searchable grid spanning the full icon set — and any theme colour for it (Settings → Profile). The chosen glyph and the profile name beside it recolour wherever the profile appears (top-bar button, profile picker), adapting to light/dark. Named icons lazy-load so the bundle stays small. (protocol 7.4)
