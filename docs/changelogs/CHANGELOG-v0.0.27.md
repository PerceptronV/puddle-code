...
- The left sidebar no longer **blanks while it re-binds**: switching between sessions that share a worktree (the default arrangement) used to drop the file tree, the git decorations, the uncommitted list, and the commit graph to "Loading…" and rebuild them. The worktree queries now hold their previous answer until the next lands, and Changes/Search key on the worktree path rather than the session id (measured: a same-worktree switch went from ~50ms of placeholders to none).
- Switching projects under project-based layout no longer **rebuilds the terminals**: the other projects' terminals stay mounted and parked instead of being disposed and replayed on return (measured: the xterm instances survive a round trip, where every one of them used to be rebuilt).
- The **first painted frame is themed**: semantic colour tokens live only in `[data-theme]` blocks and the app module is deferred, so every load flashed one unthemed (white) frame first. An inline script in `index.html` now sets the theme, density and rem base before the paint.
