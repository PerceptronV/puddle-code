...
Repos      GET  /api/fs/dirs?prefix=…        # directory autocomplete for repo registration (dirs only, dotdirs included, is_git flag)
           ...
           GET   /api/repos/:id/worktrees     # every checked-out git worktree {path, branch, is_primary}; the clone is is_primary; feeds the "directory to join" picker (§4 join_worktree)
