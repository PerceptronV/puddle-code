...
- **Markdown/HTML preview**: a `file` tab whose path is markdown (`.md`/`.markdown`/`.mdown`) or HTML (`.html`/`.htm`) grows a **source ⇄ preview toggle on its tab chip** (hover-revealed beside the close button: an eye to preview, a file-code glyph back to source). The mode persists as the tab ref's optional `view` field in `ui_state.layout_tree` — not part of tab identity, so toggling rewrites the tab in place. Markdown renders inline as theme-styled prose: `marked` (GFM) sanitised through DOMPurify before touching innerHTML; the text comes from the tab's **shared editor model**, so the preview tracks unsaved edits live; relative image references resolve against the document's directory through the authed `GET /media` → object-URL path (element loads carry no bearer header), refusing worktree-escaping `..` references; external links open a new tab, relative/anchor links are inert. HTML renders in an **iframe sandboxed as `allow-scripts` without `allow-same-origin`** — a null origin that can never reach the app's origin, storage, or daemon token — on a white `--paper` canvas in both themes (a document's own colours assume a browser-default canvas); relative assets inside the HTML iframe are deliberately unresolved in v1.
