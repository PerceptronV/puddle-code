...
**Desktop remote hosts.** File → "Connect to SSH Host…" (⌘⇧O) prompts for any ssh destination and runs the same `connectRemote()` the CLI uses — bootstrap over the master connection, tunnel, handshake — opening **one window per target** (local or `user@host`), which mirrors the CLI's one-cockpit-per-target model and the web app's one-daemon-per-origin assumption. Successful targets land in a File → Recent Hosts submenu (hostnames only, in the app's userData; never credentials — auth stays the system ssh's business). Closing a window stops that cockpit's UI server and tunnel; the remote daemon and agents run on. Known residue, shared with detached CLI cockpits: the app has no TTY for ssh to prompt on, so key-authenticated hosts (or a warm ControlMaster) connect fine while password/2FA hosts must use `puddle connect` in a terminal — the connect dialogue says so on failure.
