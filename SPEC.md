...
**Credential carve-out (subscription usage only).** Puddle reads an account's own OAuth token in exactly one place: fetching subscription rate-limit usage for the profile panel's progress bars, and only when the user has switched on per-account rate-limit tracking (off by default). The token is read from the agent's documented credential file, used solely as the bearer for that one request, never logged, stored, or proxied. Everything fails safe to "unavailable" — a missing/keychain-only token, a network error, or an unexpected response yields no data rather than wrong numbers. This is the sole exception to "never reads credentials"; all other flows (login, status, usage counts, live context/cost) touch no secrets.
...
Accounts   GET  /api/accounts?profile=…      POST /api/accounts {profile_id, agent_type, label, skip_permissions_default?, import_dir?}   # import_dir: copy a pre-existing config dir (§2)
           PATCH /api/accounts/:id {skip_permissions_default?, rate_limit_tracking?}   # §11 gate opt-in; subscription-usage opt-in (reads the token — §2 carve-out)
           ...
           GET  /api/accounts/:id/usage      # session counts + last activity (puddle); best-effort agent token totals; live_usage (context fill %, cost) via the status line; subscription rate-limit windows when opted in — all nullable
