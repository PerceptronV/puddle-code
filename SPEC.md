...
Repos      GET  /api/fs/dirs?prefix=…        # directory autocomplete for repo registration (dirs only, dotdirs included, is_git flag)
           ...
           GET   /api/repos/:id/worktrees     # every checked-out git worktree {path, branch, is_primary, dirty, local_only}; the clone is is_primary; feeds the join picker (§4) and worktree manager (§8)
           DELETE /api/repos/:id/worktrees?path=&confirm=   # prune a worktree dir (branch kept); refuses clone/dirty/live-session; confirm required for a local-only branch (§8)
...
- **URLs**: xterm.js `web-links` addon; plain click (or cmd+click—match both) opens in a new browser tab. In SSH mode, URLs pointing at `localhost:<port>` on the host are rewritten to the tier-2 proxy path so they work from the client; in local mode they are left untouched. The proxy those URLs target ships in Phase 5, but the rewriting itself needs the client to know it is in SSH mode, which arrives with the CLI's SSH transport — so the terminal-URL rewrite lands with Phase 6, not Phase 4.
...
- **Worktrees navigator**: a repo-wide manager (not bound to the pin) listing every git worktree of the project's repo grouped by branch (`GET /api/repos/:id/worktrees`), each tagged with its state — the **clone** (primary), **uncommitted** (dirty), **local only** (branch has commits on no remote), and a **running** count. A prune control (`DELETE /api/repos/:id/worktrees?path=&confirm=`) removes a worktree's directory (never the branch); the daemon refuses to prune the clone (`worktree_primary`), a dirty worktree (`worktree_dirty`), or one a live session is using (`worktree_busy`), and requires `confirm` for a purely-local branch (`worktree_unpushed`). The UI mirrors the block reasons (disabled control + tooltip) and shows a warning in the confirm dialog for local-only branches.
