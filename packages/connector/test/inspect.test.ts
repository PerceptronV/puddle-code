...
it('reports unconfigured hosts without creating remote state', async () => {
  ...
  try {
    ...
    expect(existsSync(join(home, 'remote'))).toBe(false);
  } finally {
...
it('projects offline host status without returning routing credentials or private configuration', async () => {
  const home = mkdtempSync(join(tmpdir(), 'puddle-remote-inspect-'));
  try {
    atomicPrivateJson(join(home, 'remote/config.json'), {
      enabled: false,
    ...
    atomicPrivateJson(join(home, 'remote/identity.json'), [...identity]);
    const status = await inspectConnector(home);
    ...
    expect(existsSync(join(home, 'remote/devices.db'))).toBe(false);
  } finally {
    rmSync(home, { recursive: true, force: true });
  }
});
