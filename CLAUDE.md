...
- Mobile access uses self-hosted Better Auth login, separately trusted application delivery, maintained Noise XX and exact host-approved browser keys. Remote protocol 1 is separate from daemon/cockpit protocol 18.1. `puddle remote` and Settings → Remote access manage host registration, pairing, approval/revocation, disable and local/SSH-only identity reset; configuration lives in `~/.puddle/remote`. The complete remote operation allowlist is `connector/src/policy.ts`. New remote operations require review there; never bypass the shared host lease boundary. See `docs/mobile-access.md` and `docs/acceptance/mobile-access.md`.
