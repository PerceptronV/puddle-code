...
- Native terminal editing on coarse-pointer devices lives in `terminal/native-input.ts`: capture on the xterm textarea’s parent (xterm already has target capture listeners), translate native replacements and selection changes into terminal input, and keep only bounded ephemeral text typed by this viewer. Never parse an agent’s prompt to reconstruct an input field. Tap placement must match that text against live cells; commands/reconnects invalidate context. Touch key controls use `mobile/TerminalKey.tsx` to suppress synthetic clicks while retaining focus.
