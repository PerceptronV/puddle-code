...
- Mobile access uses self-hosted Better Auth Google/GitHub OAuth only (at least one provider, public signup with provider-verified email, no email/password delivery or recovery), separately trusted application delivery, maintained Noise XX and exact host-approved browser keys. Remote protocol 2 is separate from daemon/cockpit protocol 21.2. Desktop-led account registration uses a five-minute browser-approved hash; its one-use verifier remains in cockpit memory and goes only to the machine status/redemption endpoints. Host identity generation and browser approval remain on the host. `puddle remote` and Settings → Remote & Sync manage host registration, pairing, approval/revocation, disable and local/SSH-only identity reset; desktop settings also delete saved registrations, revoking browsers while retaining the host identity and revoked audit records. Deletion/replacement retires the old relay record through machine-only `/remote/unregister`; private `remote/retired/` files retain at most 32 routing contexts for bounded, idempotent retry, including supervisor starts without an active config. Disable/offline hosts remain registered; never deduplicate by label. Deletion is capability-detected and never exposed through the encrypted remote admin protocol; configuration lives in `~/.puddle/remote`. The complete remote operation allowlist is `connector/src/policy.ts`. New remote operations require review there; never bypass the shared host lease boundary. See `docs/mobile-access.md` and `docs/acceptance/mobile-access.md`.
...
- Shared terminal touch gestures distinguish quick scrolling from held local selection. Selection must never send agent mouse input or copy automatically; Copy requires an explicit tap. Mobile file text uses native DOM selection with held-drag extension and the same Copy action, and its path dialogue resolves files/directories against the current browse root through the existing nil-session directory target.
