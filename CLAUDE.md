...
- Remote runtime images explicitly make package metadata, configuration and public assets readable by their unprivileged users. CI builds from a private checkout and runs `node deploy/remote/test-images.mjs` with read-only filesystems and dropped capabilities; never rely on the operator's umask or relax permissions on deployment secrets. The app's Caddy binary must have no privileged-port file capability: it serves port 8080.
